...
local SECONDARY_MUZZLE_POS = script:GetCustomProperty("SecondaryMuzzlePos")
local PRIMARY_MUZZLE_POS = script:GetPosition()
local currentMuzzle = 1
...
function Attack(target)
	...
	-- Alternate between muzzle positions
	if currentMuzzle == 1 and SECONDARY_MUZZLE_POS ~= nil then
		currentMuzzle = 2
		script:SetPosition(SECONDARY_MUZZLE_POS)
	else
		currentMuzzle = 1
		script:SetPosition(PRIMARY_MUZZLE_POS)
	end
end
